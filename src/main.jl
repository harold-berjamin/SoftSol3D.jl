...
function main()
    ...
    mesh = Mesh(
        xmin = [-0.2, -0.1, -0.1], # [-0.5, -0.1, -0.1]
        xmax = [ 0.2,  0.1,  0.1], # [ 0.5,  0.1,  0.1]
        tmax = 0.08, # 0.18
        Nx = [400, 4, 4], # [100, 2, 2], [100, 4, 4]
        Nt = 0, # 0
        Co = 0.95, # 0.95 (Courant number)
        flux = "MUSCL", # "LLF", "MUSCL"
        Nf = 6*minimum(length, [mate.g, mate.ω]) + 12 # 6*minimum(length, [mate.g, mate.ω]) + 12
    )
    Sin2(t::Float64, Ω::Float64) = 2e0 * (sin(Ω*t) - 0.5*sin(2*Ω*t)) .* ((t>=0) - (t>=2*π/Ω))
    src = Source(
        spar = SourcePar(
            type = "Plane", # "Plane", "Point"
            dir = 1, # 1
            cmp = 2 # 2
        ),
        Ω = 82.35, # 82.35
